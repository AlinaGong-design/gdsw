  ...
  // 7. 第二个失败案例（权限错误）
  {
    ...
    spans: [
      ...
      },
      {
        ...
        metadata: { toolName: 'code_repo_api', statusCode: 403, error: 'PERMISSION_DENIED' },
      },
      {
        spanId: 'e3', traceId: 'trace-007-stu901', parentSpanId: 'e1',
        name: '错误诊断', type: 'llm', status: 'success',
        startTime: 3060, duration: 380, depth: 1,
        input: '代码仓库 API 返回 403 错误，token 缺少 repo scope 权限。请生成诊断建议。',
        output: '诊断：代码仓库 Personal Access Token 未包含 repo 权限范围。解决方案：请在代码仓库 Settings > Developer settings > Personal access tokens 中为 Token 添加 repo 权限，然后在万卷平台重新配置代码仓库集成。',
